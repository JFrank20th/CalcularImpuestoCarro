...
public class CalculadorImpuestos {
    ...
    public Vehiculo buscarVehiculoMasCaro()   {
        ...
        for (int i = 0; i < vehiculos.length; i++) {
            posVehiculoActual = i;
            if (darVehiculoActual().darPrecio() >= precio){
                precio = darVehiculoActual().darPrecio();
                masCaro =   darVehiculoActual();
            }
        }
        return masCaro;
    }
    ...
    public Vehiculo buscarVehiculoPorMarca() {
        ...
        String marca = JOptionPane.showInputDialog("Marca de vehiculo a buscar");

        for (int i = 0; i < vehiculos.length; i++) {
            posVehiculoActual = i;
            if (darVehiculoActual().darMarca().toLowerCase(Locale.ROOT).equals(marca.toLowerCase(Locale.ROOT))){
    ...
    public Vehiculo buscarVehiculoPorLinea() {
        Vehiculo buscado = null;
        String linea = JOptionPane.showInputDialog("linea de vehiculo a buscar");

        for (int i = 0; i < vehiculos.length; i++) {
            posVehiculoActual = i;
            if (darVehiculoActual().darLinea().toLowerCase(Locale.ROOT).equals(linea.toLowerCase(Locale.ROOT))){
                buscado = darVehiculoActual();
            }
        }
    ...
    public Vehiculo buscarVehiculoMasAntiguo() {
        ...
        for (int i = 0; i < vehiculos.length; i++) {
            posVehiculoActual = i;
            int entero = Integer.parseInt(darVehiculoActual().darAnio());
            if (entero <= anio){
                anio = Integer.parseInt(darVehiculoActual().darAnio());
                buscado = darVehiculoActual();
            }
        }
    ...
    public double promedioPreciosVehiculos() {
        ...
        for (int i = 0; i < vehiculos.length; i++) {
            posVehiculoActual = i;
            suma = suma + darVehiculoActual().darPrecio();
            contador += 1;
        }
